...
class VirtualSelect {
  ...
  setValueText() {
    ...
    const showAllText = this.isAllSelected && !this.hasServerSearch && !this.disableAllOptionsSelectedText && !showValueAsTags;
  ...
  getVisibleOptionGroupsMapping(searchValue) {
    ...
    options.forEach(d => {
      result[d.index] = d.options.some(e => isOptionVisible({
        data: e,
        searchValue
      }).isVisible);
    });
  ...
  closeDropbox(isSilent) {
    ...
    const withinComponent = active && this.$wrapper.contains(active) || this.hasDropboxWrapper && active && this.$dropboxWrapper.contains(active);
    const shouldRefocus = this.shouldFocusWrapperOnClose && VirtualSelect.lastInteractedInstance === this && !isSilent && (active === null || active === document.body || withinComponent);
    if (shouldRefocus) {
      this.$wrapper.focus();
    }
    if (isSilent) {
  ...
  toggleAllOptions(selectAll) {
    ...
    const selectingAll = typeof isSelected === 'boolean' ? selectAll : !DomUtils.hasClass(this.$toggleAllCheckbox, 'checked');
    const selectedValues = [];
    const {
  ...
  isAllGroupOptionsSelected(groupIndex) {
    ...
    if (this.options.length) {
      isAllSelected = !this.options.some(d => !d.isSelected && !d.isDisabled && !d.isGroupTitle && d.groupIndex === groupIndex);
    }
    return isAllSelected;
  }
  ...
  isOptionVisible({
    ...
    const value = data.valueNormalized != null ? data.valueNormalized : data.value.toLowerCase();
  ...
  warnIfSecureTextDisabled() {
    ...
    // eslint-disable-next-line no-console
    console.warn('[virtual-select] Option labels and values are rendered as HTML and are NOT escaped ' + 'because `enableSecureText` is disabled (the default, kept off for performance on large ' + 'datasets). If any option text can come from untrusted input, set `enableSecureText: true` ' + 'to prevent XSS. Docs: https://sa-si-dev.github.io/virtual-select/#/properties?id=enablesecuretext');
  }
